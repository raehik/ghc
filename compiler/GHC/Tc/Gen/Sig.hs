...
tcPatSynSig name sig_ty@(L _ (HsSig{sig_bndrs = hs_outer_bndrs, sig_body = hs_ty}))
  ...
  = do { traceTc "tcPatSynSig 1" (ppr sig_ty)
       ...
       ; ze                   <- emptyZonkEnv
       ; (ze, kv_bndrs)       <- zonkTyVarBindersNoAny   ze (mkTyVarBinders InferredSpec kvs)
       ; (ze, implicit_bndrs) <- zonkTyVarBindersNoAny   ze implicit_bndrs
       ; (ze, univ_bndrs)     <- zonkTyVarBindersNoAny   ze univ_bndrs
       ; (ze, ex_bndrs)       <- zonkTyVarBindersNoAny   ze ex_bndrs
       ; req                  <- zonkTcTypesToTypesNoAny ze req
       ; prov                 <- zonkTcTypesToTypesNoAny ze prov
       ; body_ty              <- zonkTcTypeToTypeNoAny   ze body_ty
