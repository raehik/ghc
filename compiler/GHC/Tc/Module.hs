...
tcTopSrcDecls (HsGroup { hs_tyclds = tycl_decls,
 ...
 = do {         -- Type-check the type and class decls, and all imported decls
        ...
        let { all_binds = inst_binds     `unionBags`
            ...
            ; fo_fvs = foldr (\gre fvs -> fvs `addOneFV` greInternalName gre)
                                emptyFVs fo_gres
...
tcPreludeClashWarn warnFlag name = do
    ...
    ; let clashes :: GlobalRdrElt -> Bool
          clashes x = isLocalDef && nameClashes && isNotInProperModule
            where
              ...
              -- Names are identical ...
              nameClashes = nameOccName (greInternalName x) == nameOccName name
              ...
              isNotInProperModule = greInternalName x /= name
    ...
    ; let warn_msg x = addWarnAt (Reason warnFlag) (nameSrcSpan (greInternalName x)) (hsep
              [ text "Local definition of"
              , (quotes . ppr . nameOccName . greInternalName) x
