...
instance DisambECP (HsCmd GhcPs) where
  ...
  mkHsOpAppPV l c1 op c2 = do
    ...
    cs <- getCommentsFor l
    return $ L (noAnnSrcSpan l) $ HsCmdArrForm (ApiAnn (spanAsAnchor l) (AnnList Nothing Nothing [] []) cs) (reLocL op) Infix Nothing [cmdArg c1, cmdArg c2]
...
instance DisambECP (HsExpr GhcPs) where
  ...
  mkHsAppTypePV l e t anns = do
    checkExpBlockArguments e
    cs <- getCommentsFor (locA l)
    return $ L l (HsAppType (ApiAnn (spanAsAnchor $ locA l) anns cs) e (mkHsWildCardBndrs t))
  mkHsIfPV l c semi1 a semi2 b anns = do
  ...
  mkHsExplicitListPV l xs anns = do
    cs <- getCommentsFor l
    return $ L (noAnnSrcSpan l) (ExplicitList (ApiAnn (spanAsAnchor l) anns cs) Nothing xs)
  ...
  mkHsRecordPV l lrec a (fbinds, ddLoc) anns = do
    cs <- getCommentsFor l
    r <- mkRecConstrOrUpdate a lrec (fbinds, ddLoc) (ApiAnn (spanAsAnchor l) anns cs)
